...
public:
    ...
    const cv::Mat& get_img_ref() const; //121212
    // 获取帧计数
    int get_frame() const;
    // 输出图像对应的相机时间
    double get_img_t() const; //333
    ...
    double get_predict_to_send_latency() const;// 101010
    double get_send_to_control_latency() const; // 111111
    double get_control_to_fire_latency() const; //999
   ...
   * coordinate system */
    Eigen::Vector3d pc_to_pi(const Eigen::Vector3d& pc) const;//555
    // 陀螺仪 xyz -> 相机 xyz，z朝前
    Eigen::Vector3d pi_to_pc(const Eigen::Vector3d& pi) const; //444
    ...
    aimer::math::YpdCoord pu_to_yp_c(const cv::Point2f& pu) const; //141414

    cv::Point2f pu_to_pd(const cv::Point2f& pu) const;
    cv::Point2f pd_to_pu(const cv::Point2f& pd) const; // 151515

    cv::Point2f pi_to_pu(const Eigen::Vector3d& pi) const;//666
    ...
    const cv::Mat& get_f_cv_mat_ref() const { //777
        return this->f_cv_mat;
    }
    const cv::Mat& get_c_cv_mat_ref() const {
        return this->c_cv_mat;
    }
    const cv::Mat& get_rot_ic_sup_cv_mat_ref() const { //888
        return this->rot_ic_sup_cv_mat;
    }
    Eigen::Quaterniond get_q() const { //131313
        return this->q;
    }
   ...
   */
    Eigen::Vector3d xyz_i_camera_to_xyz_i_barrel(const Eigen::Vector3d& xyz_i_camera) const; //111
    Eigen::Vector3d xyz_i_barrel_to_xyz_i_camera(const Eigen::Vector3d& xyz_i_barrel) const; //222
